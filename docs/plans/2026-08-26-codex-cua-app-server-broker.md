...
**GREEN:** Implement exact path resolution, designated-requirement verification for team `2DC432GLL2`, full component vnode snapshots/recheck, minimal child environment, bounded daemon start/readiness, and exact local/running version parsing. Authenticate the connected socket with `LOCAL_PEERPID`, exact `proc_pidpath`, and Security.framework's live-code designated-requirement check. Never kill the daemon.
